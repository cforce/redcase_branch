...
function getHistory(rs)
{
	...
	txt += "<tr style='font-weight: bold; background-color: #eeeeee'><td>Erstellungsdatum</td><td>Ergebnis</td><td>Kommentar</td><td>Tester</td><td>Umgebung</td><td>Version</td></tr>";
	for(i = 0; i < rs.length; i++) {
		switch (rs[i].result)
		{
			case "Passed":
		...
		txt += "<tr" + (notFirst ? " style='background-color: " + color + "'" : " style='background-color: " + color + "; font-weight: normal'") + ">"
		txt += "<td>" + rs[i].created_on  + "</td>";
		txt += "<td>" + rs[i].result      + "</td>";
		txt += "<td>" + ( rs[i].comment == null ? '' : rs[i].comment) + "</td>";
